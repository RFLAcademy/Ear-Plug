...
"<style>"
"#feedbackRow button{font-size:22px;width:70px;}"
...
"<div id=\"feedbackRow\" style=\"display:none; margin-top:20px;\">"
  "<button onclick=\"sendFeedback('LOW')\">⬆️</button>"
  "<button onclick=\"sendFeedback('OK')\">👍</button>"
  "<button onclick=\"sendFeedback('HIGH')\">⬇️</button>"
"</div>"
...
"function sendFeedback(val){"
  "fetch('/feedback?value=' + val)"
    ".then(() => {"
      "document.getElementById('feedbackRow').style.display = 'none';"
      "setStatus.innerHTML = '✔ Feedback saved';"
    "});"
"}"
...
void setup() {
  ...
  if (!SPIFFS.begin(true)) {
    ...
    if (f) {
      f.println("time,outside_pressure,inside_pressure,pressure_difference,set_delay,feedback");
      f.close();
  ...
  server.on("/feedback", HTTP_GET, [](AsyncWebServerRequest *r){
    if (r->hasParam("value")) {
      lastFeedback = r->getParam("value")->value();
      Serial.print("📊 Feedback received: ");
      Serial.println(lastFeedback);
    }
    r->send(200, "text/plain", "OK");
  });
